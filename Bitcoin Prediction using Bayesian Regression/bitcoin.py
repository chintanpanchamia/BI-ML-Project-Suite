#GROUP-16, kbhanda2, kpalani5, psinaik, ashah7, kdesai2, cpancha
...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0, len(Xi.index)):
        temp = np.exp(wt * similarity(X, Xi.iloc[i]))
        numerator = numerator + Xi.iloc[i][len(Xi.index) - 1] * temp
        denominator = denominator + temp
    delta = numerator/denominator
    return delta"""
    meanx = np.mean(X[0:len(X)-1])
    stdx = (X[0:len(X)-1]).std()
    invariant = (X[0:len(X)-1]-meanx)
    numerator = 0.0
    denominator = 0.0
    
    for i in range(0, len(Xi)):
      meanxi = np.mean(Xi.iloc[i,0:len(X)-1])
      stdxi = (Xi.iloc[i,0:len(X)-1]).std()
      similarity = sum(invariant * (Xi.iloc[i,0:len(X)-1] - meanxi))/(stdxi * stdx * (len(X)-1))
      Temp = np.exp(wt * similarity)
      numerator = numerator + Temp * Xi.iloc[i,len(X)-1]
      denominator = denominator + Temp  
    
    return numerator/denominator

'''def similarity(X, Xi):
    meanX = np.mean(X[0 : len(X) - 2])
    meanXi = np.mean(Xi[0 : len(Xi) - 2])
    numerator = 0.0
    denominator = len(Xi.index) * (X[0 : len(X) - 2]).std() * (Xi[0 : len(Xi) - 2]).std()
    for i in range(0, len(Xi.index) - 1):
        numerator = numerator + (X[i] - meanX) * (Xi[i] - meanXi)
    return numerator/denominator'''
...
MSE = 0.0
# YOUR CODE HERE
MSE = sm.mean_squared_error(compareDF.iloc[:,0],compareDF.iloc[:,1])
