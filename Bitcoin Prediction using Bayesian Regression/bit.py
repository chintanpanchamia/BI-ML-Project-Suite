...
def computeDelta(wt, X, Xi):
    ...
    stdx=(X[0:len(X)-2]).std()
    constx=(X[0:len(X)-2]-meanx)
    #print 
    #sys.exit()
    numerator=0
    denominator=0
    for i in range(0, len(Xi)):
      meanxi=np.mean(Xi.iloc[i,0:len(X)-2])
      stdxi=(Xi.iloc[i,0:len(X)-2]).std()
      similaritySum=sum(constx*(Xi.iloc[i,0:len(X)-2]-meanxi))
      similarity=similaritySum/(stdxi*stdx*(len(X)-1))
      #print similarity
      val=np.exp(wt*similarity)
      numerator=numerator+val*Xi.iloc[i,len(X)-1]
